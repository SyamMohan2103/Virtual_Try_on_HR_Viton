...
def test(opt, test_loader, tocg, generator):
    ...
    grid_dir = os.path.join(
        "./output", opt.test_name, opt.datamode, opt.datasetting, "generator", "grid"
    )
    ...
    with torch.no_grad():
        for inputs in test_loader.data_loader:
            ...
            fake_parse_gauss = gauss(
                F.interpolate(
                    fake_segmap, size=(opt.fine_height, opt.fine_width), mode="bilinear"
                )
            )
            ...
            if opt.cuda:
                old_parse = (
                    torch.FloatTensor(
                        fake_parse.size(0), 13, opt.fine_height, opt.fine_width
                    )
                    .zero_()
                    .cuda()
                )
            ...
            if opt.cuda:
                parse = (
                    torch.FloatTensor(
                        fake_parse.size(0), 7, opt.fine_height, opt.fine_width
                    )
                    .zero_()
                    .cuda()
                )
            else:
                parse = torch.FloatTensor(
                    fake_parse.size(0), 7, opt.fine_height, opt.fine_width
                ).zero_()
            ...
            if opt.occlusion:
                ...
                warped_cloth = warped_cloth * warped_clothmask + torch.ones_like(
                    warped_cloth
                ) * (1 - warped_clothmask)
            ...
            for i in range(shape[0]):
                ...
                unpaired_name = (
                    ...
                    + "_"
                    + inputs["c_name"][opt.datasetting][i].split(".")[0]
                    + ".png"
                )
...
def process_image(image, cloth_image):
    # call the test function here
    image_fname = image.split("\\")[-1].replace('e','')
    cloth_fname = cloth_image.split("\\")[-1].replace('e','')

    with open(
        "H:\\Virtual_Try_on_HR_Viton\\data\\zalando-hd-resized\\test_pairs.txt", "w"
    ) as f:
...
with gr.Blocks() as demo:
    ...
    example_filepath = os.path.join(
        os.getcwd(), "data", "zalando-hd-resized", "test", "cloth"
    )
    gr.Markdown("## Cloth Samples")
    gr.Examples(
        examples=[
            os.path.join(example_filepath, "08532_00.jpg"),
            os.path.join(example_filepath, "00095_00.jpg"),
            os.path.join(example_filepath, "00006_00.jpg"),
            os.path.join(example_filepath, "00008_00.jpg"),
            os.path.join(example_filepath, "00013_00.jpg"),
            os.path.join(example_filepath, "00017_00.jpg"),
            os.path.join(example_filepath, "00034_00.jpg"),
            os.path.join(example_filepath, "00035_00.jpg"),
            os.path.join(example_filepath, "00055_00.jpg"),
            os.path.join(example_filepath, "00057_00.jpg"),
            os.path.join(example_filepath, "00064_00.jpg"),
            os.path.join(example_filepath, "00067_00.jpg"),
            os.path.join(example_filepath, "00069_00.jpg"),
            os.path.join(example_filepath, "00071_00.jpg"),
            os.path.join(example_filepath, "00074_00.jpg"),
            os.path.join(example_filepath, "00075_00.jpg"),
            os.path.join(example_filepath, "00084_00.jpg"),
            os.path.join(example_filepath, "00094_00.jpg"),
        ],
